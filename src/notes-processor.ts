...
export default class NotesProcessor {
	...
	public static subdivideUnderMaxBreath = (noteGroupings: Note[][], maxBreathSeconds: number): Note[][] => {
		let finalNotes: Note[][] = []
		noteGroupings.forEach((notes: Note[]) => {
			const lengthsOfEachNote = notes.map((note: Note) => note.duration)
			const recommendedSubdivisionIndicies = getPartitioningArrayWithMax(lengthsOfEachNote, maxBreathSeconds)
			finalNotes = [...finalNotes, ...sliceAndDice(notes, recommendedSubdivisionIndicies)]
		})
		return finalNotes
	}
}
